;Pipesworld with crafted valid macro ## Add crafted valid macro ## No ## The valid macro operator ``pop-start'' is added to this domain.
(define (domain pipesworld_strips_M)
...
(delm-prem-count)) 
  ...
  (:action push-end_M
   ...
   (:action push-start-push-end_M
    :parameters (?pipe - pipe ?batch-atom-in - batch-atom ?from-area - area ?to-area - area 
      ?first-batch-atom - batch-atom ?product-batch-atom-in - product ?product-first-batch - product 

      ?last-batch-atom - batch-atom ?next-last-batch-atom - batch-atom
    ...
    :precondition (and (normal_M ?pipe ) (first_M ?first-batch-atom ?pipe ) (connect_M ?from-area ?to-area ?pipe ) 
      (on_M ?batch-atom-in ?from-area ) (not-unitary_M ?pipe ) (is-product_M ?batch-atom-in ?product-batch-atom-in ) 
      (is-product_M ?first-batch-atom ?product-first-batch ) 
      (may-interface_M ?product-batch-atom-in ?product-first-batch )


      (last_M ?last-batch-atom ?pipe )  (follow_M ?last-batch-atom ?next-last-batch-atom )


       )
    :effect (and  (first_M ?batch-atom-in ?pipe ) (follow_M ?first-batch-atom ?batch-atom-in ) 
       (not (first_M ?first-batch-atom ?pipe )) (not (on_M ?batch-atom-in ?from-area ))

       (last_M ?next-last-batch-atom ?pipe ) (on_M ?last-batch-atom ?to-area )
       (not (push-updating_M ?pipe )) 

       (not (follow_M ?last-batch-atom ?next-last-batch-atom )) (not (last_M ?last-batch-atom ?pipe ))
 )
  )
