;Pipesworld with swapped atoms ## Swap between add effect and delete effect ## Yes ## The atom (last ?batch-atom-in ?pipe) in the add effects of the operator ``pop-unitarypipe'' in the original domain is swapped with the atom (connect ?from-area ?to-area ?pipe ) from the preconditions from the same operator in the original domain.
...
(define (domain pipesworld)
  ...
  (:action pop-unitarypipe_M
    :parameters (?pipe - pipe ?batch-atom-in - batch-atom ?from-area - area ?to-area - area ?last-batch-atom - batch-atom ?product-batch-atom-in - product ?product-last-batch - product )
    :precondition (and (last_M ?last-batch-atom ?pipe ) (last_M ?batch-atom-in ?pipe )  (on_M ?batch-atom-in ?to-area ) (unitary_M ?pipe ) (is-product_M ?batch-atom-in ?product-batch-atom-in ) (is-product_M ?last-batch-atom ?product-last-batch ) (may-interface_M ?product-batch-atom-in ?product-last-batch ) )
    :effect (and (first_M ?batch-atom-in ?pipe ) (on_M ?last-batch-atom ?from-area ) (connect_M ?from-area ?to-area ?pipe )  (not (last_M ?last-batch-atom ?pipe )) (not (first_M ?last-batch-atom ?pipe )) (not (on_M ?batch-atom-in ?to-area )) 
 )
  )
)
