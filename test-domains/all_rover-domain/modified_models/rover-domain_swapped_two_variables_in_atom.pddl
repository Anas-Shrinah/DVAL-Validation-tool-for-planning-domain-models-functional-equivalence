;Rover with swapped variables ## Swap positions of two variables in an atom ## Yes ## The variable ?y in the parameters of the precondition (can_traverse ?x ?y ?z ) in the operator ``navigate'' in the original domain is swapped with the variable ?z from the parameters of the same precondition of the same operator in the original domain.
...
(define (domain rover)
  ...
  (:action navigate_M
    ...
    :precondition (and (can_traverse_M ?x ?z ?y ) (available_M ?x ) (at_M ?x ?y ) (visible_M ?y ?z ) )
