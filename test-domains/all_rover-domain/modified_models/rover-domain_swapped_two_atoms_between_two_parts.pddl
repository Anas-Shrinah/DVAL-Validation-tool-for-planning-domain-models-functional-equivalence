;Rover with swapped atoms ## Swap between add effect and precondition ## Yes ## The atom (calibrated ?i ?r) in the add effects of the operator ``calibrate'' in the original domain is swapped with the atom (at ?r ?w) from the preconditions from the same operator in the original domain.
...
(define (domain rover)
  ...
  (:action calibrate_M
    ...
    :precondition (and (equipped_for_imaging_M ?r ) (calibration_target_M ?i ?t ) (visible_from_M ?t ?w ) (on_board_M ?i ?r ) (calibrated_M ?i ?r ) )
    :effect (and (at_M ?r ?w ) 
 )
  )
  (:action take_image_M
