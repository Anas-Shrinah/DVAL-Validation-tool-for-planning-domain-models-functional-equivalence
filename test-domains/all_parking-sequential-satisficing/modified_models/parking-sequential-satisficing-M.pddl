;Parking with crafted valid macro ## Add crafted valid macro ## No ## The valid macro operator ``move-car-to-curb-move-car-to-car'' is added to this domain.
...
; (:functions (total-cost) - number)
	...
	(:action move-car-to-curb-move-car-to-car
		:parameters (?car - car ?carsrc - car ?curbdest - curb  ?carsrc2 - car)
		:precondition (and 
			(car-clear ?car)
			(curb-clear ?curbdest)
			(behind-car ?car ?carsrc)

			(behind-car ?carsrc ?carsrc2)
		...
		:effect (and 
			...
			(not (car-clear ?car))
			(car-clear ?carsrc2)
			(behind-car ?carsrc ?car)
			(not (behind-car ?carsrc ?carsrc2))
                        ; (increase (total-cost) 1)
		)
	)
