;Logistics with swapped atoms ## Swap between add effect and delete effect ## Yes ## The atom (at-truck-location ?truck ?loc-to) in the add effects of the operator ``drive-truck-airport-location'' in the original domain is swapped with the atom (airport-in-city ?loc-from ?city) from the preconditions from the same operator in the original domain.
...
(define (domain logistics)
  ...
  (:action drive-truck-airport-location_M
    ...
    :precondition (and (at-truck-airport_M ?truck ?loc-from ) (at-truck-location_M ?truck ?loc-to )  (location-in-city_M ?loc-to ?city ) )
    :effect (and  (airport-in-city_M ?loc-from ?city ) (not (at-truck-airport_M ?truck ?loc-from ) ) 
 )
  )
