;Logistics with random invalid macro ## Add invalid macro ## Yes ## The invalid macro operator ``load-truck-location-load-truck-airport-load-airplane'' is added to this domain. This invalid macro is produced from a valid macro by swapping the add effect (in-truck ?x1 ?x4) from the original valid macro by the precondition (at-package-location ?x1 ?x2) from the same valid macro.
...
(define (domain logistics)
  ...
  (:action load-truck-location-load-truck-airport-load-airplane_M
    ...
    :precondition (and (at-truck-location_M ?x4 ?x2 ) (at-truck-airport_M ?x4 ?x5 ) (at-package-airport_M ?x1 ?x5 ) (at-package-airport_M ?x1 ?x6 ) (at-airplane-airport_M ?x7 ?x6 ) (in-truck_M ?x1 ?x4 ) )
    :effect (and (in-airplane_M ?x1 ?x7 ) (at-package-location_M ?x1 ?x2 ) (not (at-package-location_M ?x1 ?x2 )) (not (at-package-airport_M ?x1 ?x5 )) (not (at-package-airport_M ?x1 ?x6 )) 
 )
  )
)
