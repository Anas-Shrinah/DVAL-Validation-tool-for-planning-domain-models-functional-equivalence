;Cave-diving with random invalid macro ## Add invalid macro ## Yes ## The invalid macro operator ``prepare-tank-enter-water-pickup-tank'' is added to this domain. This invalid macro is produced from a valid macro by swapping the add effect (holding ?x1 ?x2) from the original valid macro by the precondition (capacity ?x1 ?x5) from the same valid macro.
...
(define (domain cave-diving)
  ...
  (:action prepare-tank-enter-water-pickup-tank_M
    ...
    :precondition (and (at-surface_M ?x1 ) (in-storage_M ?x2 ) (holding_M ?x1 ?x2 ) (next-quantity_M ?x4 ?x5 )  (next-tank_M ?x2 ?x3 ) (cave-entrance_M ?x6 ) (at-tank_M ?x2 ?x6 ) (next-quantity_M ?x5 ?x4 ) )
    :effect (and (in-storage_M ?x3 ) (full_M ?x2 ) (at-diver_M ?x1 ?x6 )  (capacity_M ?x1 ?x5 ) (not (in-storage_M ?x2 )) (not (at-surface_M ?x1 )) (not (at-tank_M ?x2 ?x6 )) (not (capacity_M ?x1 ?x4 ) ) 
 )
  )
)
