;Cave-diving with crafted valid macro ## Add crafted valid macro ## No ## The valid macro operator ``swim-photograph'' is added to this domain.
;; Cave Diving ADL
...
(define (domain cave-diving-adl)
  ...
  (:action swim-photograph
    :parameters (?d - diver ?t1 ?t2 - tank ?l1 ?l2 - location)
    :precondition (and (at-diver ?d ?l1)
                       (holding ?d ?t1)
                       (holding ?d ?t2)
                       (full ?t1)
  ...
  (:action decompress
    ...
    :effect (and (not (at-diver ?d ?l))
                      (decompressing ?d)
                 ; (not (in-water)) becuase it is removed with the hire action
                 ;(increase (total-cost) (other-cost ))
            )
  )
