;Cave-diving with swapped variables ## Swap positions of two variables in an atom ## Yes ## The variable ?q1 in the parameters of the precondition (next-quantity_M ?q1 ?q2 ) in the operator ``drop-tank'' in the original domain is swapped with the variable ?q2 from the parameters of the same precondition of the same operator in the original domain.
...
(define (domain cave-diving)
  ...
  (:action drop-tank_M
    :parameters (?d - diver ?t - tank ?l - location ?q1 - quantity ?q2 - quantity )
    :precondition (and (at-diver_M ?d ?l ) (holding_M ?d ?t ) (next-quantity_M ?q2 ?q1 ) (capacity_M ?d ?q1 ) )
