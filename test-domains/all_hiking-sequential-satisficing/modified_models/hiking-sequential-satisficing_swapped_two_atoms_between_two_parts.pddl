;Hiking with swapped atoms ## Swap between add effect and delete effect ## Yes ## The atom (up ?x3) in the add effects of the operator ``put_up'' in the original domain is swapped with the atom (at_tent ?x3 ?x2) from the preconditions from the same operator in the original domain.
...
(define (domain hiking)
  ...
  (:action put_up_M
    :parameters (?x1 - person ?x2 - place ?x3 - tent )
    :precondition (and (at_person_M ?x1 ?x2 )(up_M ?x3 )  (down_M ?x3 ) )
    :effect (and (at_tent_M ?x3 ?x2 ) (not (down_M ?x3 ) ) 
 )
  )
