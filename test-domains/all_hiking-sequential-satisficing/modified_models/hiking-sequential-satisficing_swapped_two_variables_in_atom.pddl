;Hiking with swapped variables ## Swap positions of two variables in an atom ## Yes ## The variable ?x3 in the parameters of the precondition (partners_M ?x6 ?x3 ?x5 ) in the operator ``walk_together'' in the original domain is swapped with the variable ?x5 from the parameters of the same precondition of the same operator in the original domain.
...
(define (domain hiking)
  ...
  (:action walk_together_M
    ...
    :precondition (and (at_tent_M ?x1 ?x2 ) (up_M ?x1 ) (at_person_M ?x3 ?x4 ) (next_M ?x4 ?x2 ) (at_person_M ?x5 ?x4 ) (walked_M ?x6 ?x4 ) (partners_M ?x6 ?x5 ?x3 ) (not (= ?x3 ?x5 )) )
