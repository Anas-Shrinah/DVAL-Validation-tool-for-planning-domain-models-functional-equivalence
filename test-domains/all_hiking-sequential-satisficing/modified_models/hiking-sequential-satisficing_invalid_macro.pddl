;Hiking with random invalid macro ## Add invalid macro ## Yes ## The invalid macro operator ``put_down-put_up-drive_passenger'' is added to this domain. This invalid macro is produced from a valid macro by swapping the add effect (at_person ?x1 ?x4) from the original valid macro by the precondition (at_tent ?x5 ?x3) from the same valid macro.
...
(define (domain hiking)
  ...
  (:action put_down-put_up-drive_passenger_M
    ...
    :precondition (and (at_person_M ?x1 ?x3 ) (at_person_M ?x1 ?x4 )  (up_M ?x5 ) (at_person_M ?x2 ?x3 ) (at_car_M ?x6 ?x3 ) (not (= ?x3 ?x5 )) )
    :effect (and (at_person_M ?x2 ?x4 ) (at_tent_M ?x5 ?x3 ) (at_car_M ?x6 ?x4 )  (not (down_M ?x5 )) (not (at_person_M ?x2 ?x3 )) (not (at_car_M ?x6 ?x3 )) (not (at_person_M ?x1 ?x3 ) ) 
 )
  )
)
