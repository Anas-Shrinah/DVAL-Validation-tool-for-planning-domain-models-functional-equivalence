;Freecell with swapped variables ## Swap positions of two variables in an atom ## Yes ## The variable ?cols in the parameters of the precondition (successor ?cols ?ncols) in the operator ``sendtohome-b'' in the original domain is swapped with the variable ?ncols from the parameters of the same precondition of the same operator in the original domain.
...
(define (domain freecell)
  ...
  (:action sendtohome-b_M
    ...
    :precondition (and (clear_M ?card ) (bottomcol_M ?card ) (home_M ?homecard ) (suit_M ?card ?suit ) (suit_M ?homecard ?suit ) (value_M ?card ?vcard ) (value_M ?homecard ?vhomecard ) (successor_M ?vcard ?vhomecard ) (colspace_M ?cols ) (successor_M ?cols ?ncols ) )
  ...
  (:action colfromfreecell_M
    ...
    :precondition (and (incell_M ?card ) (clear_M ?newcard ) (canstack_M ?card ?newcard ) (cellspace_M ?cells ) (cellspace_M ?ncells ) )
    :effect (and (clear_M ?card ) (on_M ?card ?newcard ) (successor_M ?ncells ?cells ) (not (incell_M ?card )) (not (cellspace_M ?cells )) (not (clear_M ?newcard )) 
 )
  )
