;Floor-tile with crafted valid macro ## Add crafted valid macro ## No ## The valid macro operator ``up_right'' is added to this domain.
...
(:action up_right 
  :parameters (?r - robot ?x - tile ?y - tile ?Z - tile)
  :precondition (and (robot-at ?r ?x) (up ?y ?x) (right ?z ?y) (clear ?y) (clear ?z))
  :effect (and (robot-at ?r ?z) (not (robot-at ?r ?x)) (not (robot-at ?r ?y))
               (clear ?x) (not (clear ?z))
               ; (increase (total-cost) 3)
               )
)
