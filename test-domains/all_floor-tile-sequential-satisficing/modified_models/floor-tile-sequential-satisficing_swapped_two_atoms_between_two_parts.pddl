;Floor-tile with swapped atoms ## Swap between add effect and precondition ## Yes ## The atom (clear ?x) in the add effects of the operator ``down'' in the original domain is swapped with the atom (down ?y ?x) from the preconditions from the same operator in the original domain.
...
(define (domain floor-tile)
  ...
  (:action down_M
    :parameters (?r - robot ?x - tile ?y - tile )
    :precondition (and (robot-at_M ?r ?x ) (clear_M ?y ) (clear_M ?x ) )
    :effect (and (robot-at_M ?r ?y ) (down_M ?y ?x ) (not (robot-at_M ?r ?x )) (not (clear_M ?y )) 
 )
  )
  (:action right_M
    :parameters (?r - robot ?x - tile ?y - tile )
