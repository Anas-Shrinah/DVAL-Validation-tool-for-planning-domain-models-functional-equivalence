;Child-snack with swapped atoms ## Swap between add effect and precondition ## Yes ## The atom (at ?t ?p2) in the add effects of the operator ``move_tray'' in the original domain is swapped with the atom (at ?t ?p1) from the preconditions from the same operator in the original domain.
...
(define (domain child-snack)
  ...
  (:action move_tray_M
    ...
    :precondition (and (at_M ?t ?p2 ) )
    :effect (and (at_M ?t ?p1 ) (not (at_M ?t ?p1 )) 
 )
  )
)
