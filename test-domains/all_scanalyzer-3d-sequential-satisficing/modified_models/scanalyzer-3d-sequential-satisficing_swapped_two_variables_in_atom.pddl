;Scanalyzer with swapped variables ## Swap positions of two variables in an atom ## Yes ## The variable ?s1 in the parameters of the precondition (cycle-2 ?s1 ?s2 ) in the operator ``rotate-2'' in the original domain is swapped with the variable ?s2 from the parameters of the same precondition of the same operator in the original domain.
...
(define (domain scanalyzer3d)
  ...
  (:action rotate-2_M
    :parameters (?s1 - segment ?s2 - segment ?c1 - car ?c2 - car )
    :precondition (and (cycle-2_M ?s2 ?s1 ) (on_M ?c1 ?s1 ) (on_M ?c2 ?s2 ) )
